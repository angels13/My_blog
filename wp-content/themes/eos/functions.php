...
function eos_add_theme_options() {
	...
	if(isset($_POST['eos_save_options'])) {
		...
		if ($_POST['enableFacebookIcon']) {
			$aOptions['enableFacebookIcon'] = true;
			//Facebook URL
			if (preg_match('/facebook\./i', $_POST['facebookURL'])) {
				if (!preg_match('/http[s]?\:\/\//i', $_POST['facebookURL']))
					$aOptions['facebookURL'] = 'http://'.$_POST['facebookURL'];
				else 
					$aOptions['facebookURL'] = $_POST['facebookURL'];
			} else {
				$formErrors['facebookURL'] = 'Please enter a valid facebook account URL.';
				$aOptions['facebookURL'] = '';
			}
		} else
			$aOptions['enableFacebookIcon'] = false;
...
<!-- facebook -->
				<tr valign="top">
					<th scope="row"><?php _e('Facebook', 'Eos'); ?></th>
					<td>
						<label>
							<input name="enableFacebookIcon" type="checkbox" value="checkbox" <?php if($aOptions['enableFacebookIcon']) echo "checked='checked'"; ?> />
							 <?php _e('Enable Facebook icon in sidebar', 'Eos'); ?>
						</label>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><?php _e('Facebook URL', 'Eos'); ?></th>
					<td>
						<label>
							<input name="facebookURL" class="regular-text" type="text" value="<?php if($aOptions['facebookURL']) echo $aOptions['facebookURL']; ?>"  />
							 <?php _e('Please type in the URL of your Facebook account.', 'Eos'); ?>
						</label>
					</td>
				</tr>
<!-- facebook end -->
