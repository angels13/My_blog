<?php
class analytical_lite_breadcrumb_class {
    var $opts;
    function analytical_lite_custom_breadcrumb() {
        $this->opts = array(
            'before' => '&nbsp;',
            'after' => '&nbsp; ',
            'delimiter' => '&raquo;'
        ...
        global $post, $options;
        #require('var.php');
        echo '<section class="cont_nav"><div class="cont_nav_inner"><p><a href="' . esc_url(home_url('/')) . '">' . __('Home', 'analytical-lite') . '</a>';
        if (!is_front_page()) {
            echo $markup;
        }
        ...
        $output = $this->analytical_lite_simple_breadcrumb_case($post);
    ...
    function analytical_lite_simple_breadcrumb_case($der_post) {
        $markup = $this->opts['before'] . $this->opts['delimiter'] . $this->opts['after'];
        if (is_page()) {
            if ($der_post->post_parent) {
                $my_query = get_post($der_post->post_parent);
                $this->analytical_lite_simple_breadcrumb_case($my_query);
                $link = '<a href="';
        ...
        if (is_author()) {
           global $wp_query;
            ...
            return __('Author', 'analytical-lite') . ': ' . $curauth->display_name;
        }
        if (is_tag()) {
            return __('Tag', 'analytical-lite') . ': ' . single_tag_title('', false);
        }
        if (is_search()) {
            return __('Search', 'analytical-lite');
        }
        if (is_year()) {
...
$ana_breadcumb = new analytical_lite_breadcrumb_class();
?>